		...
		for (var I = 0; I < FOON_graph.length; I++) {
			...
			if (line.startsWith("//")) {
				...
				if (newObject != null) {
					...
					if (line.includes("!")){
						nodes_lvl3[objectIndex].setGoalNode();
						console.log("hehehe");
					}
					...
					if (line.includes("!")){
						nodes_lvl2[objectIndex].setGoalNode();
					}
					...
					if (line.includes("!")){
						nodes_lvl1[objectIndex].setGoalNode();
					}
...
function FOONtoJSON(FOON) {
	...
	for (FU = 0; FU < FOON.length; FU++) {
		...
		for (U = 0; U < tempList.length; U++) {
			...
			// checking if a node is a goal node -- highlighted  with a "!" symbol
			if (tempList[U].isGoalNode()){ 	objectName += " <!>"; }
			...
			if (found == false) {
				// NOTE: type == 6 means that it is a goal node; type == 7 is a container; else, it is a regular node:
				obj.nodes.push( (objectName.includes("<!>") ? { id: objectName, type: 6 } : objectName.includes("contains") ? { id: objectName, type: 7 } : { id: objectName, type: 1 }) );
			}
		}
...
function FOONtoJSON_onemode(FOON) {
	...
	for (FU = 0; FU < FOON.length; FU++) {
		...
		for (U = 0; U < tempList.length; U++) {
			...
			if (found == false) {
				// NOTE: type == 6 means that it is a goal node; else, it is a regular node:
				obj.nodes.push(function () {if (objectName.includes("<!>")) return { id: objectName, type: 6 }; return { id: objectName, type: 1 }});
			}
		}
		tempList = FOON[FU].getOutputList();
		for (U = 0; U < tempList.length; U++) {
		...
		for (F = 0; F < tempList.length; F++) {
			...
			for (U = 0; U < target_list.length; U++) {
				...
				// checking if a node is a goal node -- highlighted  with a "!" symbol
				if (target_list[U].isGoalNode()){ 	tgt += " <!>"; }
...
function drawProgressLine(progress_line, S) {
	...
	// .enter()
	// .append("p")
	// .text(function(d) { return d.ingredient + " " + d.ground_truth + " " + d.generated;});
	...
	.text(function(d) { return "Ground truth state: " + d.state.ground_truth })
	.append("p")
	.attr("y", function(d, i) {
		return 40 + i * 40 
	  })
	.text(function(d) { return "Generated state: " + d.state.generated })
	.append("p")
	.attr("y", function(d, i) {
		return 40 + i * 40 
	  })
	.text(function(d) { return "Ground truth location: " + d.location.ground_truth })
	.append("p")
	.attr("y", function(d, i) {
		return 40 + i * 40 
	  })
	.text(function(d) { return "Generated location: " + d.location.generated })
	...
	// .enter()
  	// .append("p")
	// .text(function(d) { return d.location.ground_truth})
	// .style("background-color", function(d, i) { return i % 2 ? "#eee" : "#ddd"; })
    // .selectAll("p")
    // .data(function(d) { return d.location.ground_truth})
    // .enter().append("p")
...
function drawD3Graph(graph, S) {
	...
	var simulation = d3.forceSimulation()
		.force("link", d3.forceLink().id(function (d) { return d.id; }))
		.force("charge", d3.forceManyBody().strength(-500)) // adjust for comfortable spacing
		.force("center", d3.forceCenter(width / 2, height / 2));
	...
	var link = svg.append("g")
		...
		.data(graph.links)
		.enter().append("line")
		.attr("stroke", function() { if (show_text) return "#9D9D9D"; return "#000000";} )
		//.attr("stroke", "#000000")	// for creating universal FOON images
		.attr("marker-end", "url(#arrow)");
		//.attr("stroke-width", "1.25px");
	...
	var node = svg.append("g")
		...
		.enter()
		// uncomment this line if you want to use circles only:
		// NOTE: type 6 -- goal node; type 7 -- container; type 1 -- regular object node
		.filter(function (d) { return [1, 6, 7].includes(d.type); })
		.append("circle")
		.attr("r", function(d) {var radius = 12; if (!show_text) radius = 28; if (d.type == 6 || d.type == 7) return radius + 2; return radius;}) 
		// 12 for regular graph images; 28 for universal FOON images
		.style("fill", function(d) { 
			if (d.type == 1 || d.type == 7) return "#99FF55"; 
			else if (d.type == 2) return "#E74C3C"; 
			else if (d.type == 6) return "#2A630B"; 
			else if (d.type == 7) return "#eaf54c";
		}) // #adff2f
		.call(d3.drag()
	...
	const mot_node = svg.append("g")
		...
		.data(graph.nodes)
		.enter()
		// NOTE: type 6 -- goal node; type 1 -- regular object node
		.filter(function (d) { return !([1, 6, 7].includes(d.type));})
		.append("rect")
		// NOTE: for regular FOON graphs, use the following:
		.attr("rx", function() { if (show_text) return 6; return 18;}).attr("ry", function() { if (show_text) return 6; return 18;})
		.attr("width", square_length).attr("height", square_length)
		// NOTE: for universal FOON images (without any text), use the following:
		// .attr("rx", 18).attr("ry", 18) 
		// .attr("width", 48).attr("height", 48)
		.style("fill", function (d) {if (d.type == 2) return "#D40000"; else if (d.type == 3) return "#0066FF"; else if (d.type == 4) return "#4400AA";}) // "#E74C3C"
		.call(d3.drag()
			.on("start", dragstarted)
			.on("drag", dragged)
			.on("end", dragended))
		...
		.each(function (d) {
			...
			for (var i = 0; i < lines.length; i++) {
				d3.select(this)
					...
					.style("font-family", "monospace")
					.style("font-weight", function () { if (i == 0) return "bold"; return "semi-bold"; })
					.style("font-size", function (d) { if (!show_text) return "0em"; if (d.type == "1" && (lines[i].includes(":") || lines[i].includes("<"))) return "0.8em"; return "0.9em" }) // -- original!
					//.style("font-size", "0em")
					.text(lines[i]);
			}
		});
